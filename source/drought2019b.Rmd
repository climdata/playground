...
for (m in c(1:12)) {
  ...
  df <- data.frame(x = hdispi[,hdiCol], y = hdispi[,spiCol], ti = hdispi$ti, t1 = hdispi$t1, t2 = hdispi$t2)
  df <- subset(df, !is.na(df$x))
  mx <- lm(y ~ (x+ti+t1+t2)^2, df);  
  slope <- unname(coef(mx)[2])
  offset <- unname(coef(mx)[1])
  
  predx <- seq(-3, +3, len=50)
  pred <- predict(mx, newdata=df, se.fit=TRUE)  
...
for (m in c(1:12)) {
  ...
  # even more exact reconstruction
  mx <- mxs[[m]]
  df <- data.frame(x = hi[,hdiCol], ti = hi$ti, t1 = hi$t1, t2 = hi$t2)
  pred <- predict(mx, newdata=df, se.fit=TRUE)
  spi <- pred$fit
